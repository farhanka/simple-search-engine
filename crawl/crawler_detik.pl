...
HERE: for (my $i=0; $i<=$total_days; $i++){ #variabel $input harus dicasting ke int agar looping berjalan
    ...
    for ($page=1; $page<=10; $page++){
        #url ini adalah pola index url dari kompas.com
        $url = "https://news.detik.com/indeks/$page?date=$month/$day/$year";
        $mech->get($url);
        @all_urls = $mech->links();
        ...
        foreach my $link (@all_urls){
            my $url = $link->url;
                ...
                # saring url berita
                    ...
                    if (($url =~"https://news.detik.com/berita/"  || $url =~"https://news.detik.com/berita-") 
