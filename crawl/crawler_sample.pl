...
for (my $i=0; $i<=$total_days; $i++){ #variabel $input harus dicasting ke int agar looping berjalan
    ...
    for ($page=0; $page<=15; $page+=15){
        #url ini adalah pola url dari situs bolasport.com
        $url = "https://index.okezone.com/bydate/index/$year/$month/$day/$page";
        ...
        foreach my $link (@all_urls){
            my $url = $link->url;
                ...
                if ($url=~"/read/" && !exists $news_links{$link->url}){
                    ...
                    if ($url =~"bola.okezone.com" && $total_bola<=7500){
                      $news_links{$url} = 1;
                      $total_bola++;
                    }
                    if ($url=~"sports.okezone.com" && $total_sport<=1828){
                        $news_links{$url} = 1;
                        $total_sport++;
                    }
                }
...
print "Berhasil mendapatkan ". ($total_sport+$total_bola) ."tautan.\nMemulai pengunduhan....";
sleep(8);

foreach (sort keys (%news_links)) {
    print OUT "$_\n";
}
print "\nproses selesai!\nURL yang disimpan ". ($total_sport+$total_bola) ." \n\n";
close OUT;